...
if ($step == 1 && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['email'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($email_sent_result === true) {
            $stmt_update = $conn->prepare("UPDATE owners SET otp_code = ?, otp_expires_at = ? WHERE email = ?");
            $stmt_update->bind_param("sss", $otp, $otp_expires_at, $email);
            $stmt_update->execute();
            
            $_SESSION['reset_email_owner'] = $email;
            header("Location: reset_password_owner.php?step=2");
            exit();
        } else {
        ...
        $message = "No verified owner account found with that email address.";
        $message_type = 'danger';
    }
}
...
if ($step == 2 && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['otp'])) {
    ...
    if (strlen($new_password) < 6) {
        ...
        $stmt = $conn->prepare("SELECT id, otp_code, otp_expires_at FROM owners WHERE email = ?");
        $stmt->bind_param("s", $email);
        $stmt->execute();
        $result = $stmt->get_result();
        if ($user = $result->fetch_assoc()) {
            if ($user['otp_expires_at'] < date("Y-m-d H:i:s")) {
                ...
                $stmt_update = $conn->prepare("UPDATE owners SET password = ?, otp_code = NULL, otp_expires_at = NULL WHERE id = ?");
                $stmt_update->bind_param("si", $hashed_password, $user['id']);
                if ($stmt_update->execute()) {
                    unset($_SESSION['reset_email_owner']);
...
<head>
    ...
    <title>Reset Owner Password - Mantralayam Rooms Booking</title>
    <!-- Bootstrap CSS CDN -->
...
<body>
    <div class="form-card">
        ...
        <?php if ($step == 1): ?>
            ...
            <p class="text-muted mb-4">Enter your owner email address to receive an OTP.</p>
            <?php if($message && $message_type == 'danger'): ?><p class="alert alert-danger"><?php echo $message; ?></p><?php endif; ?>
            <form method="POST" action="reset_password_owner.php?step=1">
                ...
                <div class="d-grid"><button type="submit" class="btn btn-success btn-lg">Send OTP</button></div>
            </form>
        <?php elseif ($step == 2): ?>
            <h1 class="mb-3">Reset Your Password</h1>
            <p class="alert alert-info">An OTP has been sent to <strong><?php echo htmlspecialchars($email); ?></strong>.</p>
            <?php if($message): ?><p class="alert alert-<?php echo $message_type; ?>"><?php echo $message; ?></p><?php endif; ?>
            <form method="POST" action="reset_password_owner.php?step=2">
                ...
                <div class="d-grid"><button type="submit" class="btn btn-success btn-lg">Reset Password</button></div>
            </form>
        <?php elseif ($step == 3): ?>
            <h1 class="text-success mb-3">Success!</h1>
            <p class="alert alert-success"><?php echo $message; ?></p>
            <div class="d-grid"><a href="login_owner.php" class="btn btn-success btn-lg">Go to Owner Login</a></div>
        <?php endif; ?>
